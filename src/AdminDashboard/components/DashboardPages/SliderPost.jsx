   ...
   const refetchSlider = async () => {
    ...
    const onSubmit = async (data) => {
        try {
          ...
          if (selectedsliderId) {
            const result = await updatesliderPhoto(token, selectedsliderId, data); // Call updateTeacher function with selectedNavbarId and data
            toast.success('photo updated successfully!');
          } else {
            // Create new teacher
            const result = await createsliderPhoto({ token, formData: data });
      ...
      const handleUpdate = async (sliderId) => {
        try {
          ...
          const token = "your_token_here"; // Replace with actual token retrieval logic
          const teacherInfo = await readAllsliderPhoto(token, sliderId); // Implement a function to fetch teacher information
    ...
    const handleRemove = async (sliderId) => {
        try {
          const token = "your_token_here"; // Replace with actual token retrieval logic
          await deletesliderPhoto(token, sliderId); // Call deleteTeacher function with sliderId
    ...
    return (
        <div>
           <h2 className="text-gray-900 border-l-4 border-[#1eb2a6] pl-2 lg:text-2xl md:text-xl text-lg font-semibold capitalizemy-4">Hero Section Slider</h2>
             <h2 className="text-gray-900  mx-auto md:w-[30%] text-center w-[50%] my-6 border-x-2 border-[#1eb2a6] px-2 lg:text-2xl md:text-xl text-lg font-semibold ">Hero Section Form</h2> 

            <form  onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md md:mb-20 mb-10 md:px-3 px-1 md:py-3 py-1">
     
     <div className="grid md:grid-cols-3 grid-cols-2 gap-2">
       <div >
          <label className="font-semibold md:text-md text-sm"> tittle</label>
     ...
     <button className="md:ml-0 ml-4 md:px-2 px-1 rounded md:py-2 py-1 md:mt-4 my-2 md:w-[200px] w-[140px] hover:bg-white gap-2 border border-[#1EB2A6] hover:text-[#1EB2A6] bg-[#1EB2A6] text-white md:text-md text-sm flex justify-center items-center" 
      type="submit">{selectedsliderId ? "Update slider photo" : "Create slider photo"}</button>
    
   </form>
  <h2 className="text-gray-900 mx-auto md:w-[30%]  w-[70%] text-center mt-6 border-x-2 border-[#1eb2a6] px-2 lg:text-2xl md:text-xl text-lg font-semibold ">Hero Section Image</h2> 
